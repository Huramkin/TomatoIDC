...
use App\GoodsInfoModel;
use App\Goods;
use App\Http\Requests\AdminGoodsPost;
use App\User;
...
class GoodsController extends Controller
{
    ...
    {
        $goodsConfigure = new GoodsConfigureController();
        switch ($type) {
            case "virtual_host":
    ...
    public function goodChargingAdd(Request $request)
    {
        ...
        switch ($request['type']) {
            case 'multicycle' :
                ...
                break;
            case 'disposable':
                Goods::where('id', $request['id'])->update(['price' => $request['price']]);
                break;
            case 'month_price':
                Goods::where('id', $request['id'])->update(['month_price' => $request['price']]);
                break;
        }
...
//        dd($request['type']);
        ...
        switch ($request['type']) {
            ...
            case 'disposable':
                ...
                Goods::where('id', $request['id'])->update(['price' => 9999]);
                break;
            case 'month_price':
                $this->validate($request, [
                    'id' => 'exists:goods,id|required'
                ]);
                Goods::where('id', $request['id'])->update(['month_price' => null]);
                break;
            default:
                return response('error', 500);
        }
    ...
    public function goodCategoriesEditAction(Request $request)
    {
        ...
        GoodsCategories::where('id', $request['id'])->update(
            [
                'title' => $request['title'],
                'subtitle' => $request['subtitle'],
                'display' => $request['display'],
    ...
    public function goodCategoriesDelAction(Request $request)
    {
        ...
        GoodsCategories::where('id', $request['id'])->update(['status' => 0]);
        return redirect(route('admin.good.show'));
    }
    ...
    /**
     * 商品添加操作
     * @param AdminGoodsPost $request
     * @return RedirectResponse|\Illuminate\Routing\Redirector
     */
    public function goodAddAction(AdminGoodsPost $request)
    {
        AdminController::checkAdminAuthority(Auth::user());
        ...
        !empty($purchase_limit) ?: $purchase_limit = 0;
        !empty($upgrade) ?: $upgrade = 0;

        GoodsInfoModel::create(
            [
                'goods_id' => $goods->id,
                'level' => $request['level'],
                'display' => $request['display'],
     ...
     * @return RedirectResponse|\Illuminate\Routing\Redirector
     */
    public function goodEditAction(AdminGoodsPost $request)
    {
        AdminController::checkAdminAuthority(Auth::user());
