...
class PleskController extends Controller
{
    ...
    protected function createApiKey($server)
    {
        $resp = $this->sendPost($server,['content-type' => 'application/json'],'auth/keys');
        dd($resp);
    }

    protected function sendPost($server, $params = [],$action = "swagger.yml")
    {
        $publicParams = [
            'Authorization' => $server->username.':'.$server->password,
            'headers' => ['Authorization' => $server->username.':'.$server->password],
            'form_params' => [
...
//        dd($arr);
        $url = $this->getUrl($server) . '/api/v2/'.$action;
//        try {
            $client = new Client();
            $resp = $client->request('POST', $url, $arr);
//        } catch (RequestException $e) {
//            Log::error('Plesk error ', [$e, $params]);
//            return false;
//        }
