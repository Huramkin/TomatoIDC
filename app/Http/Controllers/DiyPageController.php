...
class DiyPageController extends Controller
{
    ...
    public function indexPage($hash)
    {
        ...
        if (DB::table('diy_pages')->where([['hash', $hash], ['status', 1]])->exists()) {
            $page = DB::table('diy_pages')->where([['hash', $hash], ['status', 1]])->first();
    ...
    public function diyPageEditAction(Request $request)
    {
        ...
        DB::table('diy_pages')->where('hash', $request['hash'])->update(
            ['hash' => $request['hash'], 'content' => $request['content']]
        );
        return redirect(route('admin.diy.page.show'));
    }
    ...
    public function diyPageDelAction(Request $request)
    {
        ...
        $this->validate(
            $request, [
                'hash' => 'exists:diy_pages,hash|required'
            ]
        );
        DB::table('diy_pages')->where('hash', $request['hash'])->update(['status' => 0]);
        return redirect(route('admin.diy.page.show'));
    }
}
